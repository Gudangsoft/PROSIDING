<div>
    <div class="max-w-7xl mx-auto px-4 py-8">
        ...
        @if(session('success'))
        <div class="mb-4 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm flex items-center gap-2">✅ {{ session('success') }}</div>
        @endif
        @if(session('error'))
        <div class="mb-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm flex items-center gap-2">❌ {{ session('error') }}</div>
        @endif

        {{-- Tabs --}}
        <div class="flex gap-1 mb-6 border-b border-gray-200">
            @foreach(['settings' => 'Konfigurasi', 'templates' => 'Template Pesan', 'reminder' => 'Kirim Reminder', 'logs' => 'Log Pengiriman'] as $tab => $label)
            <button wire:click="$set('activeTab', '{{ $tab }}')"
                class="px-4 py-2.5 text-sm font-medium border-b-2 -mb-px transition {{ $activeTab === $tab ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                {{ $label }}
                @if($tab === 'logs' && $logStats['today'] > 0)
                <span class="ml-1 inline-flex px-1.5 py-0.5 text-[10px] font-bold bg-green-100 text-green-700 rounded-full">{{ $logStats['today'] }}</span>
                @endif
            </button>
            @endforeach
        </div>
        ...
        <div class="bg-green-50 border border-green-200 rounded-2xl p-4 mb-5 flex items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-sm font-semibold text-green-800">Aktif: {{ $activeSetting->name }}
                        <span class="ml-2 px-2 py-0.5 text-[10px] font-bold uppercase rounded-full bg-green-200 text-green-800">{{ $activeSetting->provider_label }}</span>
                    </p>
                    <p class="text-xs text-green-600">Sender: {{ $activeSetting->sender_number ?? 'tidak dikonfigurasi' }}</p>
                </div>
            </div>
            <button wire:click="testSend({{ $activeSetting->id }})" class="px-3 py-1.5 bg-green-600 text-white text-xs font-semibold rounded-lg hover:bg-green-700 transition">
                🔔 Test Kirim
            </button>
        </div>
        @else
        ...
        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 {{ $s->is_active ? 'bg-green-50/30' : '' }}">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ $s->name }}</td>
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2 py-0.5 text-xs rounded-full
                                {{ $s->provider === 'fonnte' ? 'bg-blue-100 text-blue-700' : ($s->provider === 'wablas' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-700') }}">
                                {{ $s->provider_label }}
                            </span>
                        </td>
                        <td class="px-4 py-3 hidden md:table-cell text-gray-500 font-mono text-xs">
                            {{ $s->api_key ? substr($s->api_key, 0, 8) . '••••••••' : '-' }}
                        </td>
                        <td class="px-4 py-3 hidden lg:table-cell text-gray-600 text-xs">{{ $s->sender_number ?? '-' }}</td>
                        <td class="px-4 py-3 text-center">
                            <button wire:click="toggleActive({{ $s->id }})" title="{{ $s->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center gap-1">
                                <button wire:click="testSend({{ $s->id }})" class="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition font-medium">Test</button>
                                <button wire:click="openEdit({{ $s->id }})" class="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition font-medium">Edit</button>
                                <button wire:click="deleteSetting({{ $s->id }})" wire:confirm="Hapus konfigurasi ini?" class="px-2 py-1 text-xs bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition font-medium">Hapus</button>
                            </div>
                        </td>
                    </tr>
                    @empty
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            <div class="bg-white border border-gray-200 rounded-2xl p-4 text-center">
                <p class="text-2xl font-bold text-gray-800">{{ $logStats['total'] }}</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            @foreach(\App\Models\WhatsappSetting::TEMPLATES as $key => $label)
            @php
            ...
            <div class="bg-white rounded-2xl border border-gray-200 p-4">
                <div class="flex items-center justify-between mb-2">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <button wire:click="resetTemplate('{{ $key }}')" class="text-[10px] text-gray-400 hover:text-orange-500 transition">Reset</button>
                        @else
                        <span class="text-[10px] px-1.5 py-0.5 bg-gray-100 text-gray-500 rounded-full">Default</span>
                        @endif
                        <button wire:click="openTemplate('{{ $key }}')" class="px-2 py-0.5 text-xs bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition font-medium">Edit</button>
                    </div>
                </div>
                <p class="text-xs text-gray-500 whitespace-pre-line line-clamp-4 font-mono bg-gray-50 rounded-lg px-3 py-2 leading-relaxed">{{ $currentTpl }}</p>
                <div class="mt-2 flex flex-wrap gap-1">
                    @foreach($this->getTemplateVars($key) as $var)
                    <code class="text-[10px] bg-green-50 text-green-700 px-1.5 py-0.5 rounded border border-green-100">&#123;{{ $var }}&#125;</code>
                    @endforeach
                </div>
            </div>
            @endforeach
                ...
                <div class="space-y-3">
                    ...
                    <div class="p-3 bg-green-50 rounded-xl border border-green-200 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-xl font-bold text-green-700">{{ $targetCount }} orang</p>
                        </div>
                    ...
                    <button wire:click="sendPaymentReminders" wire:confirm="Kirim reminder ke {{ $targetCount }} orang? Proses ini akan memakan beberapa waktu."
                        ...
                        @disabled($targetCount === 0 || !$activeSetting)>
                        <span wire:loading.remove wire:target="sendPaymentReminders">📤 Kirim Reminder ({{ $targetCount }})</span>
        ...
        <div class="flex flex-wrap gap-3 mb-4 items-center justify-between">
            <div class="flex flex-wrap gap-2 flex-1">
                ...
                <select wire:model.live="logType" class="px-3 py-2 border border-gray-300 rounded-xl text-sm">
                    ...
                    @foreach(\App\Models\WhatsappLog::TYPES as $k => $v)
            ...
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            <p class="font-medium text-gray-800 text-xs">{{ $log->recipient_name ?? $log->to }}</p>
                            <p class="text-[11px] text-gray-400 font-mono">{{ $log->to }}</p>
                        </td>
                        <td class="px-4 py-3 hidden md:table-cell">
                            <span class="inline-flex px-2 py-0.5 text-[10px] rounded-full bg-gray-100 text-gray-600 font-medium">{{ $log->type_label }}</span>
                        </td>
                        <td class="px-4 py-3 hidden lg:table-cell text-xs text-gray-500 max-w-[200px] truncate">{{ $log->message }}</td>
                        <td class="px-4 py-3 text-center">
                        ...
                        <td class="px-4 py-3 hidden md:table-cell text-xs text-gray-400">{{ $log->created_at->format('d/m/y H:i') }}</td>
                        <td class="px-4 py-3 text-center">
                            <button wire:click="deleteLog({{ $log->id }})" class="text-red-400 hover:text-red-600 transition">
    ...
    <div class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-5 border-b border-gray-200">
                <h3 class="text-lg font-bold text-gray-800">{{ $editId ? 'Edit' : 'Tambah' }} Konfigurasi WhatsApp</h3>
            ...
            <div class="p-5 space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        @error('settingName') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        ...
                        <select wire:model.live="provider" class="w-full px-3 py-2 border border-gray-300 rounded-xl text-sm">
                            @foreach(\App\Models\WhatsappSetting::PROVIDERS as $k => $v)
                            <option value="{{ $k }}">{{ $v }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">
                        API Key / Token
                        @if($provider === 'fonnte') <span class="text-gray-400 font-normal">— dari dashboard fonnte.com</span>
                        @elseif($provider === 'wablas') <span class="text-gray-400 font-normal">— dari dashboard wablas.com</span>
                        @endif
                    </label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">
                        API URL
                        <span class="text-gray-400 font-normal">{{ $provider === 'wablas' ? 'e.g. https://solo.wablas.com/api/send-message' : 'URL endpoint custom' }}</span>
                    </label>
                ...
                <div class="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        ...
                        <label class="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                            <input wire:model="{{ $prop }}" type="checkbox" class="w-4 h-4 text-green-600 rounded">
                            {{ $label }}
                        </label>
                        @endforeach
                    </div>
        ...
        <div class="bg-white rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div class="p-5 space-y-4">
                ...
                <div>
                    ...
                    @error('sendToNumber') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                ...
                <div>
                    ...
                    @error('sendMessage') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>
            </div>
            <div class="flex justify-end gap-3 p-5 border-t border-gray-200">
